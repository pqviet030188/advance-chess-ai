...
) (move *bitboard.Bitboard, attack *bitboard.Bitboard, build *bitboard.Bitboard) {
	// get all moves
	allMoves := bitboard.Nearby(square, model.FactMask)
	if allMoves == nil {
		panic(fmt.Sprintf("Cannot look up nearby moves for builder square: %d", square))
	}
